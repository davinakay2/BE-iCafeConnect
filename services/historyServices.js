...
const insertICafeBillingHistory = async (
  transactionId,
  userId,
  billingPriceId
) => {
...
const displayICafeBillingHistory = async (userid) => {
  try {
    const [rows] = await db.execute(
      "SELECT * FROM icafe_billing_history WHERE userid = ? ORDER BY date DESC",
      [userid]
    );
    ...
    rows.forEach((rows) => {
      console.log(`Transaction ID: ${rows.icafe_transaction_id}`);
      console.log(`User ID: ${rows.userid}`);
      console.log(`iCafe Name: ${rows.icafe_name}`);
      console.log(`Date: ${rows.date}`);
      console.log(`Hours: ${rows.hours}`);
      console.log(`PC Category: ${rows.pc_category}`);
      console.log(`Payment Method: ${rows.payment_method}`);
      console.log(`Price: ${rows.price}`);
      console.log("------------------------");
    });
    return rows;
  } catch (error) {
...
const displayEwalletTopupHistory = async (userid) => {
  try {
    const [rows] = await db.execute(
      "SELECT ewallet_transaction_id, userid, date, topup_amount, payment_method  FROM ewallet_topup_history WHERE userid = ? ORDER BY date DESC",
      [userid]
    );
    ...
    console.log("eWallet Top-up History:");
    rows.forEach((rows) => {
      console.log(`Transaction ID: ${rows.ewallet_transaction_id}`);
      console.log(`User ID: ${rows.userid}`);
      console.log(`Date: ${rows.date}`);
      console.log(`Top-up Amount: ${rows.topup_amount}`);
      console.log(`Payment Method: ${rows.payment_method}`);
      console.log("------------------------");
    });
    return rows;
